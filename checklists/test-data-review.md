...
- [ ] The Gate authority source is known when this pack is release/task-required.
- [ ] Target exact SHA / validation profile is recorded when the pack is formal Validation evidence.
...
- [ ] Hidden Validation execution is not claimed before Candidate Freeze when the project requires frozen-candidate execution.
- [ ] Status uses PASS / FAIL / NOT_RUN / NOT_APPLICABLE / BLOCKED.
- [ ] Pack PASS is not being used to substitute for another required Gate.
