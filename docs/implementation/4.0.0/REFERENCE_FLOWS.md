...
8. unresolved valid P0/P1 findings block aggregate PASS by severity; a contradictory `blocking=false` cannot waive blocker class.
...
11. a model-diverse claim must satisfy the exact diversity basis declared by its Assurance Plan; label changes alone are not diversity.
...
implementation (PRODUCE)
  ...
  -> blind reviewer A (ASSURE, model-diverse-adversarial)
  -> blind reviewer B (ASSURE, model-diverse-adversarial)
...
The Assurance Plan expands policy/mode/coverage/independence/aggregation explicitly. When separate blind lanes are materialized, they must declare and satisfy a machine-verifiable model-diversity basis such as `provider-diverse` or `model-family-diverse`. Model diversity is a risk control, not executable evidence.
...
Every verdictive Release Qualification state remains bound to one FROZEN candidate SHA/tree. Missing mandatory evidence remains NOT_RUN/BLOCKED and cannot be manufactured by DECIDE.

Hidden shared metadata is public-safe metadata only; allow-listed field names do not permit structured fixture/oracle payloads to be smuggled inside an allowed value.
...
Fast Path is ineligible when scope contains material public-contract, architecture, security/trust, migration/recovery, concurrency/exactly-once, cross-repository/authority, unknown-validation, unresolved blocker, **unresolved authority contradiction**, model-diverse/coherence, nontrivial Execution Pack or material dependency-graph concerns.
...
- duplicate `finding_id` inputs and ghost finding refs fail closed;
- P0/P1 severity dominance cannot be weakened through duplicate/superseded linkage;
- malformed canonical semantic input fails closed with structured errors rather than becoming PASS;
...
| RF-04 High-risk | PRODUCE/ASSURE/CONTROL | two blind model-diverse lanes + specialist as needed | exact contract/code subject | model agreement != evidence |
