...
T-006/T-009 own executable reducer/schema enforcement.
...
```text
current decision: extend/reuse event-v2 additively
...
| CHALLENGE | no dedicated required event today | may be additive if T-004 proves need |
...
T-005 freezes correlation/envelope semantics and the `event-v2 additive extension` decision only. It does not implement schema changes, reducer logic, new event enums or transport software. Those belong to T-004/T-006/T-009 as applicable.
