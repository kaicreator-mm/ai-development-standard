...
Version: 4.0.0
Dogfood subject baseline: `version/v4.0.0@21301c89dd48542238a06c6264cb715f33f0648d`

## 1. Purpose

This protocol dogfoods v4's own Assurance / adversarial-review semantics against the v4 architecture and machine-contract set.
...
ref: version/v4.0.0
subject_sha: 21301c89dd48542238a06c6264cb715f33f0648d
```
...
T-010 branch changes are not part of the dogfood subject. This avoids evidence becoming stale merely because T-010 records the evidence.
...
```yaml
assurance_plan_id: dogfood:v4:t010
operation_id: op:v4:t010:dogfood
subject_identity_ref: sha:21301c89dd48542238a06c6264cb715f33f0648d
activities:
  - assurance_id: dogfood:blind-a
    kind: review
    policy: required
    mode: model-diverse-adversarial
    coverage: [authority, operation, assurance, aggregation, exchange, routing, validation-release, fast-path, machine-contract]
    independence:
      context: required
      model: required
      executor: required
      evidence: none
  - assurance_id: dogfood:blind-b
    kind: review
    policy: required
    mode: model-diverse-adversarial
    coverage: [authority, operation, assurance, aggregation, exchange, routing, validation-release, fast-path, machine-contract]
    independence:
      ...
      executor: required
      evidence: none
  - assurance_id: dogfood:aggregate
    kind: review
    policy: required
    mode: cross-peer
    depends_on: [dogfood:blind-a, dogfood:blind-b]
aggregation:
...
| findings | find silent finding erasure, identity collision, invalid duplicate/superseded resolution |
| aggregation | find any majority-vote or blocker-waiver route |
...
| Validation | find any PASS without actual execution or identity binding |
| Candidate/Release | find collapse of preparation/freeze/hidden/release/integration |
| Fast Path | find reduced-truth path disguised as reduced ceremony |
| machine contract | find prose invariant not enforced or schema/verifier contradiction |
| compatibility | find v3.4 invariant weakened by v4 |
...
3. preserve the full finding union;
4. explicitly disposition every P2 and every P3 when complete disposition is selected;
5. keep every unresolved valid P0/P1 in `unresolved_blocker_refs`;
6. reject duplicate finding IDs;
7. keep `requested_route_authority = NON_AUTHORITATIVE_DERIVED_STATE`;
8. never use majority vote for correctness;
9. record any executable Validation request rather than inventing runtime truth;
10. yield one of `PASS | CHANGES_REQUESTED | VALIDATION_REQUESTED | BLOCKED`.

## 9. Durable GitHub evidence layout

T-010 uses GitHub durable records rather than hidden chat state:

```text
Dogfood coordinator Issue
...
Dogfood PASS for T-010 requires:

- two blind first-pass results meeting the declared context/model/executor independence;
- full coverage or explicit non-PASS for missing coverage;
- durable finding union and conflict/disposition records;
- no unresolved P0/P1;
- exact subject identity preserved;
- aggregation satisfying current T-009 machine semantics;
- any runtime claim supported by owning executable evidence rather than review consensus.

T-010 completion additionally requires normal exact-head repository validation and required Fresh Independent Review of the T-010 PR itself. Dogfood review is evidence about the v4 subject; it does not replace PR Review or CI.
