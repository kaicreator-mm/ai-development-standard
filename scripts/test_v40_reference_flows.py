...
R3_BASELINE = "7a8c3c3608bc42325a1ac3c15a335747dea25ddb"
...
class V40ReferenceFlowTests(unittest.TestCase):
    ...
    def test_r3_subject_is_post_r2_hardening_and_prior_dogfoods_stay_historical(self) -> None:
        self.assertEqual(self.data["subject_baseline"], R3_BASELINE)
        self.assertEqual(self.data["hardening_merge"], R3_BASELINE)
        historical = {(item["coordinator"], item["outcome"]) for item in self.data["historical_dogfood"]}
        self.assertEqual(
            historical,
            {("#127", "CHANGES_REQUESTED"), ("#135", "CHANGES_REQUESTED")},
        )
    ...
    def test_fast_path_elides_ceremony_not_truth_and_authority_conflict_escalates(self) -> None:
        ...
        self.assertIn("unresolved authority contradiction", scenario["non_substitution"])
        self.assertIn("AUTHORITY_CONTRADICTION", scenario["failure_routes"])
    ...
    def test_r3_dogfood_plan_is_blind_provider_diverse_and_not_majority_voting(self) -> None:
        dogfood = self.data["dogfood"]
        self.assertEqual(dogfood["subject_identity_ref"], f"sha:{R3_BASELINE}")
        self.assertEqual(dogfood["assurance_plan_id"], "dogfood:v4:t010:r3")
        self.assertEqual(dogfood["operation_id"], "op:v4:t010:dogfood:r3")
        self.assertEqual(dogfood["blind_first_pass_count"], 2)
        self.assertEqual(
            dogfood["blind_assurance_ids"],
            ["dogfood:r3:blind-a", "dogfood:r3:blind-b"],
        )
        self.assertEqual(dogfood["mode"], "model-diverse-adversarial")
        self.assertEqual(dogfood["model_diversity_basis"], "provider-diverse")
        ...
        self.assertEqual(
            set(dogfood["provenance_fields_required"]),
            {"provider","model_family","model_id","executor_id","context_ref","blind_first_pass_ref"},
        )
        self.assertFalse(dogfood["majority_vote_for_correctness"])
        self.assertTrue(dogfood["coverage_attestation_required"])
        self.assertTrue(dogfood["p0_p1_blocker_by_severity"])
        self.assertTrue(dogfood["p2_p3_explicit_disposition"])
        self.assertEqual(dogfood["aggregation"], "finding-union-blocker-dominance")
        self.assertEqual(
