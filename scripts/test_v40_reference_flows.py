...
class V40ReferenceFlowTests(unittest.TestCase):
    ...
    def test_r2_subject_is_post_hardening_and_prior_dogfood_stays_historical(self) -> None:
        self.assertEqual(
            self.data["subject_baseline"],
            "123c80420c6ae519e18647a76b30e489d52a8fe7",
        )
        self.assertEqual(self.data["historical_dogfood"]["coordinator"], "#127")
        self.assertEqual(self.data["historical_dogfood"]["outcome"], "CHANGES_REQUESTED")
        self.assertEqual(
            self.data["historical_dogfood"]["hardening_merge"],
            self.data["subject_baseline"],
        )
    ...
    def test_dogfood_plan_is_blind_model_diverse_hardened_and_not_majority_voting(self) -> None:
