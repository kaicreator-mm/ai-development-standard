...
class V40FinalHardeningTests(unittest.TestCase):
    ...
    def test_duplicate_finding_identity_shadow_is_rejected(self) -> None:
        shadow = copy.deepcopy(
            self.examples["forbidden"]["duplicate_finding_identity_shadow"]
        )
        aggregate = aggregation_example()
        aggregate["finding_refs"].append("F-SHADOW")
        errors = validate_review_aggregation(
            aggregate,
            base_findings() + shadow,
            plan=assurance_example(),
        )
        self.assertTrue(
            any("duplicate finding identities are forbidden: F-SHADOW" in error for error in errors),
            errors,
        )
    ...
    def test_valid_supersession_can_resolve_old_blocker(self) -> None:
        ...
        self.assertEqual(
            validate_review_aggregation(
                aggregate,
                base_findings() + [old, replacement],
                plan=assurance_example(),
            ),
            [],
        )

    def test_superseded_missing_linkage_fails_closed(self) -> None:
        old = superseded_blocker("F-OLD", None)
        aggregate = aggregation_example()
        ...
        self.assertTrue(any("superseded_by" in error for error in errors), errors)
        self.assertTrue(any("PASS forbidden" in error for error in errors), errors)

    def test_superseded_missing_target_fails_closed(self) -> None:
        old = superseded_blocker("F-OLD", "F-NOT-THERE")
        aggregate = aggregation_example()
        aggregate["finding_refs"].append("F-OLD")
        errors = validate_review_aggregation(
            aggregate,
        ...
        self.assertTrue(any("target does not exist" in error for error in errors), errors)
        self.assertTrue(any("PASS forbidden" in error for error in errors), errors)

    def test_superseded_self_link_fails_closed(self) -> None:
        old = superseded_blocker("F-OLD", "F-OLD")
        aggregate = aggregation_example()
        aggregate["finding_refs"].append("F-OLD")
        errors = validate_review_aggregation(
            aggregate,
        ...
        self.assertTrue(any("cannot target itself" in error for error in errors), errors)

    def test_superseded_cross_subject_link_fails_closed(self) -> None:
        ...
        self.assertTrue(any("crosses subject identity" in error for error in errors), errors)

    def test_superseded_cycle_fails_closed(self) -> None:
        a = superseded_blocker("F-A", "F-B")
        b = superseded_blocker("F-B", "F-A")
        aggregate = aggregation_example()
        aggregate["finding_refs"] += ["F-A", "F-B"]
        errors = validate_review_aggregation(
            aggregate,
            base_findings() + [a, b],
            plan=assurance_example(),
    ...
    def test_unresolved_supersession_replacement_remains_visible_blocker(self) -> None:
        ...
        self.assertTrue(any("PASS forbidden" in error for error in errors), errors)
        self.assertTrue(any("F-NEW" in error for error in errors), errors)
    ...
    def test_event_v2_handoff_accepts_additive_correlation_fields(self) -> None:
        ...
        event = {
            ...
            "sha": SHA_A,
            "evidence": "issue:#123",
            "operation_id": "op:T009:hardening",
            "exchange_id": "ex:T009:hardening",
            "assurance_id": "review:T009:hardening",
            "caused_by": "dispatch:T009:hardening",
        }
    ...
    def test_semantic_action_key_changes_when_precondition_changes(self) -> None:
        ...
        self.assertNotEqual(
            canonical_semantic_action_key(base),
            canonical_semantic_action_key(changed),
        )
    ...
    def test_project_defined_identity_error_explains_nonweakening_failure(self) -> None:
        ...
        self.assertTrue(
            any("weakens" in error or "missing sha" in error for error in errors),
            errors,
        )
